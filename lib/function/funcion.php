...
{
	if(empty($where)) $condicion=" where $tabla.reg_eli=0 "; else $condicion= $where." and $tabla.reg_eli=0 ";
	$db= new  Database();
	$sql="select ".$valor.", ".$nombre." as nombre  from ".$tabla." ".$condicion." order by ".$orden;
	$db->query($sql);
	echo "<label class='cField gSelect'><select name='".$nombre_obj."' id='".$nombre_obj."' $evento >";
...
function editar_registro_where($tabla,$compos,$condicion,$interfaz) {
	$complemto_campos=",fec_modif='".date("Y-m-d h:i:s")."', usu_acce='".$_SESSION["global"][2]."' ";
	$sql="UPDATE  $tabla set $compos $complemto_campos where reg_eli=0 and $condicion ";
	$db= new  Database();
	$db->query($sql);
	$id=$db->insert_id();
	...
	auditoria ($tabla,2,$sql,$interfaz,$id);
	return $id;
}
...
function combo_sql($nombre_obj,$valor,$nombre,$valor_edicion,$sql) 
{
$db= new  Database();
$db->query($sql);
echo " <select name='".$nombre_obj."'  id='".$nombre_obj."' class='SELECT'>";
...
function validar_ingreso($usu,$pass){
	$db= new  Database();
	$pass=encode_this_get($pass);
	$sql="SELECT * FROM  usuario inner join  perfil on usuario.cod_per_usu=perfil.cod_per    WHERE log_usu='$usu'  AND pas_usu='$pass' and usuario.reg_eli=0 ";
	//exit;
	$db->query($sql);
	if($db->next_row()){
		session_start();
		$_SESSION["global"][1] = $db->nom_usu;//nombre del usuario
		$_SESSION["global"][2] = $db->cod_usu;//codigo del usuario
		$_SESSION["global"][3] = str_replace("@","",$db->ima_usu);//imagen de perfil del usuario
		return 1;
	}
	else{
		return 0;
...
function combo_tabla($nombre_obj, $tabla, $valor, $nombre, $valor_edicion, $filtro="", $orden="") 
{
	...
	$db->query($sql);
	echo "<label class='cField gSelect'><select name='".$nombre_obj."' id='".correcion_html_utf8($nombre_obj)."'>";
	while($db->next_row()) {
		if($valor_edicion==$db->$valor) 
			echo" <option value=".$db->$valor." selected='selected'>".$db->$nombre."</option>";
		else
...
function conteo_registro_sql_group ($tabla, $condicion, $groupby) {
	$db= new  Database();
	$sql="SELECT * FROM  $tabla  WHERE reg_eli!=1 $condicion GROUP BY $groupby";
	$db->query($sql);
	return $db->num_rows();
	$db->close();
}
...
function encode_this_get_correo($string) 
{
	$control = "1azsx1"; //defino la llave para encriptar la cadena, cambiarla por la que deseamos usar
	$string = $control.$string.$control; //concateno la llave para encriptar la cadena
	$string = base64_encode($string);//codifico la cadena
...
function decode_get2_get_correo($string) 
{
	$cadena222 = combinarcadenaatras(ltrim($string));
...
function coversion_html_utf8_espcial_ca($String){	
...
function formato_fecha($fecha,$op){ // nombre_dia |  nombre_mes | dia | mes | ano  
	if(!empty($fecha) and !empty($op) )
	{
		...
		if ($dia=="Wednesday") $dia_nombre="mi&eacute;rcoles";
		if ($dia=="Thursday") $dia_nombre="jueves";
		if ($dia=="Friday") $dia_nombre="viernes";
		if ($dia=="Saturday") $dia_nombre="s&aacute;bado";
...
function subirArchivo($nombre_certificado, $carpeta)
{
	if(!empty($nombre_certificado['name']))
	{
		...
		$un= uniqid();
		$nombre_archivo=$un.'.'.$extension;
		$valida=validarExtension($extension);
		if($valida==0){
			if(!empty($nombre_certificado['name']))
			{
				copy($nombre_certificado['tmp_name'],$carpeta.$nombre_archivo); 
			}
		}
...
function acortar_url($url) {
	...
	$url_corta = file_get_contents($apiurl);
	return $url_corta;
}
...
function buscar_registro_where ($campo, $tabla, $condicion ){
	$db= new  Database();
	  $sql="SELECT $campo as campo_nombre  FROM  $tabla WHERE reg_eli=0 $condicion  ";
	$db->query($sql);
	if($db->next_row()){
		return $db->campo_nombre; 
	}
...
function coversion_html_hexa($String, $state){
	if($state=='lowercase'){
		$String=str_replace("&#193;","&#225;",$String);//á;
		$String=str_replace("&#201;","&#233;",$String);//é
		$String=str_replace("&#205;","&#237;",$String);//í
		$String=str_replace("&#211;","&#243;",$String);//ó
		$String=str_replace("&#218;","&#250;",$String);//ú;
		$String=str_replace("&#209;","&#241;",$String);//ñ;
...
function consultaParametros($tabla,$columna,$where){
...
function combo_evento_inner($nombre_obj,$tabla,$valor,$nombre,$valor_edicion,$evento, $where ,$orden,$inner) {
	if(empty($where)) $condicion=" where $tabla.reg_eli=0 "; else $condicion= $where." and $tabla.reg_eli=0 ";
	$db= new  Database();
	$sql="select ".$valor.", ".$nombre." as nombre  from ".$tabla." ".$inner." ".$condicion." order by ".$orden;
	$db->query($sql);
	echo " <select name='".$nombre_obj."' id='".$nombre_obj."' class='SELECT'  $evento  >";
	echo "<option value='0' selected='selected'>Seleccione..</option>";
	while($db->next_row()) {
...
function contarRegistro($tabla,$where){
...
function agregar_recurso2($obj, $id_registro,$tabla,$tipo_proyecto){
	if(!empty($obj)){

		$peliculas=str_replace("{", "", $obj);
		$peliculas=str_replace("}", "", $peliculas);
		$peliculas=explode(",",$peliculas);

		$campos="(cod_pro,cod_det_pro,tip_pro,orden)";
		$j=1;

		for ($i=0 ; $i<count($peliculas) ; $i++){
			$peliculasA=explode(":",$peliculas[$i]);
			$valores="('".$id_registro."','".$peliculasA[1]."','".$tipo_proyecto."','".$j."')" ;

			$ultimo_iddet=insertar($tabla,$campos,$valores,''); 
			$j++;
		}
	}
}

function agregar_recurso($obj, $id_registro,$tabla){
	if(!empty($obj)){
		...
		$campos="(cod_pro,cod_equi,orden)";
		$j=1;

		for ($i=0 ; $i<count($peliculas) ; $i++){
			$peliculasA=explode(":",$peliculas[$i]);
			$valores="('".$id_registro."','".$peliculasA[1]."','".$j."')" ;
...
function borrar_recursos($tabla, $id_registro, $columna,$filtro){
	$db= new  Database();
	$sql="delete from $tabla  where $columna='$id_registro'  $filtro";
	$db->query($sql);
}

// Guardar Tabla Multimedia
function guardar_multimedia($files){
	// Variables de archivo
	$name = $files['name']; $new_name = $files['name']; $old_name = $files['old_name']; $size = $files['size']; $type = $files['type'][0].'/'.$files['type'][1]; $url_dir = $files['url_dir']; $file = $files['file'];
	$campos = "nom_mul,nom_old_mul,type_mul,tamano,url_dir,file_mul";
	$valores = "'".$name."','".$old_name."','".$type."','".$size."','".$url_dir."','".$file."'";
	insertar_registros('multimedia', $campos, $valores, '');
}

// Consultar Tabla Multimedia
function consultar_multimedia($nom_files){
	if(!empty($nom_files)) {
		$new_nom_files = str_replace("|", ",", $nom_files);
		$new_nom_files = str_replace("@", "'", $new_nom_files);
		$db= new  Database();
		$sql="select * from multimedia where nom_mul in ($new_nom_files) and reg_eli=0";
		$db->query($sql);
		$json1 = array();
		while($db->next_row()){
			$json2 = array(
				'name' => $db->nom_mul,
				'new_name'=> $db->nom_mul,
				'old_name'=> $db->nom_old_mul,
				'size'=> $db->tamano,
				'type'=> $db->type_mul,
				'url_dir'=> $db->url_dir,
				'file'=> $db->url_dir.$db->nom_mul
			);
			array_push($json1, $json2);
		}
		return $json1 = json_encode($json1);
	}
	else { return ''; }
}

// Eliminar Tabla Multimedia
function borrar_multimedia($nom_file){
	editar_registro('multimedia', 'reg_eli=1', 'nom_mul', $nom_file, '');
}
?>